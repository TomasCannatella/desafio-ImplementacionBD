...
func (h *InvoicesDefault) UpdateInvoicesTotal() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		invoiceUpdated, err := h.sv.UpdateInvoicesTotal()
		if err != nil {
		...
		if invoiceUpdated == 0 {
			response.JSON(w, http.StatusOK, map[string]any{
				"message": "the invoice totals were already updated",
			})
		} else {
			message := fmt.Sprintf("%d invoices totals were updated", invoiceUpdated)
			response.JSON(w, http.StatusOK, map[string]any{
				"message": message,
			})
		}
